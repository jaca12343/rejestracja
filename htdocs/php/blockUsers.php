<?php
    ...
    echo "zablokowano następujące emaile:";
    foreach ($registeredUsers as $user) {
        echo "<br>".mysqli_fetch_array(mysqli_query($conn, "SELECT email FROM `uzytkownicy` WHERE `id` = $user;"))['email'];
        mysqli_query($conn, "UPDATE `uzytkownicy` SET `zablokowany` = '1' WHERE `id` = $user;");
    }
    foreach ($unregisteredUsers as $user) {
        ...
        mysqli_query($conn, "UPDATE `rejestrowani_uzytkownicy` SET `zablokowany` = '1' WHERE `id` = $user;");
    }
